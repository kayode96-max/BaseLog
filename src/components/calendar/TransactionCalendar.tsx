      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-7 mb-4">
          {weekDays.map((day) => (
            <div
              key={day}
              className="text-center text-sm font-medium text-gray-400 dark:text-gray-500 py-2"
            >
              {day}
            </div>
          ))}
          ...
          {calendarDays.map((day) => {
            ...
            return (
              <button
                key={dateKey}
                onClick={() =>
                  hasTransactions && onDateClick(day, dayTransactions)
                }
                ...
                className={`
                  ...
                  ${
                    isToday
                      ? "bg-blue-50/50 dark:bg-blue-900/10 ring-1 ring-primary/30"
                      : ""
                  }
                `}
              >
                ...
                {hasTransactions && (
                  <div className="flex flex-col items-center gap-1 mt-1">
                    <div className="flex items-center gap-0.5">
                      ...
                      {Array.from({
                        length: Math.min(dayTransactions.length, 3),
                      }).map((_, i) => (
                        <div
                          key={i}
                          className={`w-1 h-1 rounded-full ${
                            dayTransactions[i]?.type === "sent"
                              ? "bg-red-400"
                              : "bg-green-400"
                          }`}
                        />
